...
describe "/polls/edit.html.haml" do
  ...
  it "should have these crumbs" do
    ...
    session[:crumbs].should have_crumb_to(edit_poll_path(@poll)).named("Editing Poll")
  end
  
  it "should have this title" do
    template.should_receive(:title_block).and_return("Editing poll")
    do_get
  end
  ...
  describe " -- the options partial" do
    ...
    it "should render the option partial twice" do
      template.should_receive(:render).with(:partial => 'option_form', :locals => {:option => []})
      do_get
    end
